...
int
enc_RobinFilenames(unsigned char *p, size_t len, const RobinFilenames *data, size_t *size)
{
size_t ret = 0;
size_t l;
int i, e;
...
int oldret = ret;
ret = 0;
e = enc_RobinFilename(p, len, &(data)->val[i], &l);
BACK;
ret += oldret;
}
...
int
dec_RobinFilenames(const unsigned char *p, size_t len, RobinFilenames *data, size_t *size)
{
...
e = dec_RobinFilename(p, len, &(data)->val[(data)->len-1], &l);
FORW;
len = origlen - ret;
}
ret += oldret;
...
return 0;
fail:
free_RobinFilenames(data);
return e;
}

void
free_RobinFilenames(RobinFilenames *data)
{
while ((data)->len){
free_RobinFilename(&(data)->val[(data)->len-1]);
(data)->len--;
}
free((data)->val);
(data)->val = NULL;
...
size_t
len_RobinFilenames(const RobinFilenames *data)
{
size_t ret = 0;
{
int oldret = ret;
...
int oldret = ret;
ret = 0;
ret += len_RobinFilename(&(data)->val[i]);
ret += oldret;
}
...
int
copy_RobinFilenames(const RobinFilenames *from, RobinFilenames *to)
{
if (((to)->val = malloc((from)->len * sizeof(*(to)->val))) == NULL && (from)->len != 0)
return ENOMEM;
for ((to)->len = 0; (to)->len < (from)->len; (to)->len++){
if (copy_RobinFilename(&(from)->val[(to)->len], &(to)->val[(to)->len])) return ENOMEM;
}
return 0;
}

RobinFilename *
add_RobinFilename(RobinFilenames *data)
{
	RobinFilename *this;
	if (!data) return NULL;
...
int
del_RobinFilename(RobinFilenames *data, RobinFilename *del)
{
	unsigned int u;
	RobinFilename *this;
