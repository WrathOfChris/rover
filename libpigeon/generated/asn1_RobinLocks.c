...
int
enc_RobinLocks(unsigned char *p, size_t len, const RobinLocks *data, size_t *size)
{
size_t ret = 0;
size_t l;
int i, e;
...
int oldret = ret;
ret = 0;
e = enc_RobinLock(p, len, &(data)->val[i], &l);
BACK;
ret += oldret;
}
...
int
dec_RobinLocks(const unsigned char *p, size_t len, RobinLocks *data, size_t *size)
{
...
e = dec_RobinLock(p, len, &(data)->val[(data)->len-1], &l);
FORW;
len = origlen - ret;
}
ret += oldret;
...
return 0;
fail:
free_RobinLocks(data);
return e;
}

void
free_RobinLocks(RobinLocks *data)
{
while ((data)->len){
free_RobinLock(&(data)->val[(data)->len-1]);
(data)->len--;
}
free((data)->val);
(data)->val = NULL;
...
size_t
len_RobinLocks(const RobinLocks *data)
{
size_t ret = 0;
{
int oldret = ret;
...
int
copy_RobinLocks(const RobinLocks *from, RobinLocks *to)
{
if (((to)->val = malloc((from)->len * sizeof(*(to)->val))) == NULL && (from)->len != 0)
return ENOMEM;
for ((to)->len = 0; (to)->len < (from)->len; (to)->len++){
if (copy_RobinLock(&(from)->val[(to)->len], &(to)->val[(to)->len])) return ENOMEM;
}
return 0;
}

RobinLock *
add_RobinLock(RobinLocks *data)
{
	RobinLock *this;
	if (!data) return NULL;
...
int
del_RobinLock(RobinLocks *data, RobinLock *del)
{
	unsigned int u;
	RobinLock *this;
