...
int
enc_RobinPrincipal(unsigned char *p, size_t len, const RobinPrincipal *data, size_t *size)
{
size_t ret = 0;
size_t l;
int i, e;
...
int
dec_RobinPrincipal(const unsigned char *p, size_t len, RobinPrincipal *data, size_t *size)
{
...
return 0;
fail:
free_RobinPrincipal(data);
return e;
}

void
free_RobinPrincipal(RobinPrincipal *data)
{
free_str(&(data)->name);
free_str(&(data)->cell);
}
...
size_t
len_RobinPrincipal(const RobinPrincipal *data)
{
size_t ret = 0;
{
int oldret = ret;
ret = 0;
ret += len_str(&(data)->name);
ret += 1 + len_len(ret) + oldret;
}
{
int oldret = ret;
ret = 0;
ret += len_str(&(data)->cell);
ret += 1 + len_len(ret) + oldret;
}
ret += 1 + len_len(ret);
return ret;
...
int
copy_RobinPrincipal(const RobinPrincipal *from, RobinPrincipal *to)
{
if (copy_str(&(from)->name, &(to)->name)) return ENOMEM;
if (copy_str(&(from)->cell, &(to)->cell)) return ENOMEM;
return 0;
}
