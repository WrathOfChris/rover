...
int
enc_ROBIN_GETDISK(unsigned char *p, size_t len, const ROBIN_GETDISK *data, size_t *size)
{
size_t ret = 0;
size_t l;
int i, e;
...
BACK;
e = tb_der_put_len_and_tag(p, len, ret, ASN1_C_APPL, CONS, 3, &l);
BACK;
*size = ret;
return 0;
}
...
int
dec_ROBIN_GETDISK(const unsigned char *p, size_t len, ROBIN_GETDISK *data, size_t *size)
{
...
reallen = 0;
e = tb_der_match_tag_and_len(p, len, ASN1_C_APPL, CONS, 3, &reallen, &l);
FORW;
{
int dce_fix = 0;
...
return 0;
fail:
free_ROBIN_GETDISK(data);
return e;
}

void
free_ROBIN_GETDISK(ROBIN_GETDISK *data)
{
...
size_t
len_ROBIN_GETDISK(const ROBIN_GETDISK *data)
{
size_t ret = 0;
{
int oldret = ret;
...
int
copy_ROBIN_GETDISK(const ROBIN_GETDISK *from, ROBIN_GETDISK *to)
{
