...
int
enc_RobinSignature(unsigned char *p, size_t len, const RobinSignature *data, size_t *size)
{
size_t ret = 0;
size_t l;
int i, e;
...
int oldret = ret;
ret = 0;
e = enc_bytes(p, len, &(data)->sigdata, &l);
BACK;
...
int oldret = ret;
ret = 0;
e = enc_int32(p, len, &(data)->keytype, &l);
BACK;
e = tb_der_put_len_and_tag(p, len, ret, ASN1_C_CONTEXT, CONS, 1, &l);
BACK;
ret += oldret;
...
int oldret = ret;
ret = 0;
e = enc_TOKENTYPE(p, len, &(data)->sigtype, &l);
BACK;
...
int
dec_RobinSignature(const unsigned char *p, size_t len, RobinSignature *data, size_t *size)
{
...
e = dec_TOKENTYPE(p, len, &(data)->sigtype, &l);
FORW;
if (dce_fix){
...
e = dec_int32(p, len, &(data)->keytype, &l);
FORW;
if (dce_fix){
e = tb_der_match_tag_and_len(p, len, (Der_class)0, (Der_type)0, 0, &reallen, &l);
FORW;
...
e = dec_bytes(p, len, &(data)->sigdata, &l);
FORW;
if (dce_fix){
e = tb_der_match_tag_and_len(p, len, (Der_class)0, (Der_type)0, 0, &reallen, &l);
FORW;
...
return 0;
fail:
free_RobinSignature(data);
return e;
}

void
free_RobinSignature(RobinSignature *data)
{
free_TOKENTYPE(&(data)->sigtype);
free_bytes(&(data)->sigdata);
}

size_t
len_RobinSignature(const RobinSignature *data)
{
size_t ret = 0;
{
int oldret = ret;
ret = 0;
ret += len_TOKENTYPE(&(data)->sigtype);
ret += 1 + len_len(ret) + oldret;
}
{
int oldret = ret;
ret = 0;
ret += len_int32(&(data)->keytype);
ret += 1 + len_len(ret) + oldret;
}
{
int oldret = ret;
ret = 0;
ret += len_int32(&(data)->kvno);
ret += 1 + len_len(ret) + oldret;
}
{
int oldret = ret;
ret = 0;
ret += len_bytes(&(data)->sigdata);
ret += 1 + len_len(ret) + oldret;
}
ret += 1 + len_len(ret);
return ret;
...
int
copy_RobinSignature(const RobinSignature *from, RobinSignature *to)
{
if (copy_TOKENTYPE(&(from)->sigtype, &(to)->sigtype)) return ENOMEM;
*(&(to)->keytype) = *(&(from)->keytype);
*(&(to)->kvno) = *(&(from)->kvno);
if (copy_bytes(&(from)->sigdata, &(to)->sigdata)) return ENOMEM;
return 0;
}
