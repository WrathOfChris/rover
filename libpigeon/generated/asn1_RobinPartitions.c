...
int
enc_RobinPartitions(unsigned char *p, size_t len, const RobinPartitions *data, size_t *size)
{
size_t ret = 0;
size_t l;
int i, e;
...
int oldret = ret;
ret = 0;
e = enc_RobinPartition(p, len, &(data)->val[i], &l);
BACK;
ret += oldret;
}
...
int
dec_RobinPartitions(const unsigned char *p, size_t len, RobinPartitions *data, size_t *size)
{
...
e = dec_RobinPartition(p, len, &(data)->val[(data)->len-1], &l);
FORW;
len = origlen - ret;
}
ret += oldret;
...
return 0;
fail:
free_RobinPartitions(data);
return e;
}

void
free_RobinPartitions(RobinPartitions *data)
{
while ((data)->len){
free_RobinPartition(&(data)->val[(data)->len-1]);
(data)->len--;
}
free((data)->val);
(data)->val = NULL;
...
size_t
len_RobinPartitions(const RobinPartitions *data)
{
size_t ret = 0;
{
int oldret = ret;
...
int oldret = ret;
ret = 0;
ret += len_RobinPartition(&(data)->val[i]);
ret += oldret;
}
...
int
copy_RobinPartitions(const RobinPartitions *from, RobinPartitions *to)
{
if (((to)->val = malloc((from)->len * sizeof(*(to)->val))) == NULL && (from)->len != 0)
return ENOMEM;
for ((to)->len = 0; (to)->len < (from)->len; (to)->len++){
if (copy_RobinPartition(&(from)->val[(to)->len], &(to)->val[(to)->len])) return ENOMEM;
}
return 0;
}

RobinPartition *
add_RobinPartition(RobinPartitions *data)
{
	RobinPartition *this;
	if (!data) return NULL;
...
int
del_RobinPartition(RobinPartitions *data, RobinPartition *del)
{
	unsigned int u;
	RobinPartition *this;
