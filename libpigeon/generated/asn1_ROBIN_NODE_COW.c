...
int
enc_ROBIN_NODE_COW(unsigned char *p, size_t len, const ROBIN_NODE_COW *data, size_t *size)
{
size_t ret = 0;
size_t l;
int i, e;
...
int oldret = ret;
ret = 0;
e = enc_RobinHandle(p, len, &(data)->nhdl, &l);
BACK;
...
BACK;
e = tb_der_put_len_and_tag(p, len, ret, ASN1_C_APPL, CONS, 62, &l);
BACK;
*size = ret;
return 0;
}
...
int
dec_ROBIN_NODE_COW(const unsigned char *p, size_t len, ROBIN_NODE_COW *data, size_t *size)
{
...
reallen = 0;
e = tb_der_match_tag_and_len(p, len, ASN1_C_APPL, CONS, 62, &reallen, &l);
FORW;
{
int dce_fix = 0;
...
e = dec_RobinHandle(p, len, &(data)->nhdl, &l);
FORW;
if (dce_fix){
e = tb_der_match_tag_and_len(p, len, (Der_class)0, (Der_type)0, 0, &reallen, &l);
FORW;
...
return 0;
fail:
free_ROBIN_NODE_COW(data);
return e;
}

void
free_ROBIN_NODE_COW(ROBIN_NODE_COW *data)
{
free_RobinHeader(&(data)->hdr);
free_RobinNodeFlags(&(data)->flags);
free_RobinHandle(&(data)->hdl);
free_RobinHandle(&(data)->nhdl);
if ((data)->disk) {
...
size_t
len_ROBIN_NODE_COW(const ROBIN_NODE_COW *data)
{
size_t ret = 0;
{
int oldret = ret;
...
int
copy_ROBIN_NODE_COW(const ROBIN_NODE_COW *from, ROBIN_NODE_COW *to)
{
...
if (copy_RobinHandle(&(from)->nhdl, &(to)->nhdl)) return ENOMEM;
if ((from)->disk) {
