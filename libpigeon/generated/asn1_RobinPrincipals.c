...
int
enc_RobinPrincipals(unsigned char *p, size_t len, const RobinPrincipals *data, size_t *size)
{
size_t ret = 0;
size_t l;
int i, e;
...
int oldret = ret;
ret = 0;
e = enc_RobinPrincipal(p, len, &(data)->val[i], &l);
BACK;
ret += oldret;
}
...
int
dec_RobinPrincipals(const unsigned char *p, size_t len, RobinPrincipals *data, size_t *size)
{
...
e = dec_RobinPrincipal(p, len, &(data)->val[(data)->len-1], &l);
FORW;
len = origlen - ret;
}
ret += oldret;
...
return 0;
fail:
free_RobinPrincipals(data);
return e;
}

void
free_RobinPrincipals(RobinPrincipals *data)
{
while ((data)->len){
free_RobinPrincipal(&(data)->val[(data)->len-1]);
(data)->len--;
}
free((data)->val);
(data)->val = NULL;
...
size_t
len_RobinPrincipals(const RobinPrincipals *data)
{
size_t ret = 0;
{
int oldret = ret;
...
int oldret = ret;
ret = 0;
ret += len_RobinPrincipal(&(data)->val[i]);
ret += oldret;
}
...
int
copy_RobinPrincipals(const RobinPrincipals *from, RobinPrincipals *to)
{
if (((to)->val = malloc((from)->len * sizeof(*(to)->val))) == NULL && (from)->len != 0)
return ENOMEM;
for ((to)->len = 0; (to)->len < (from)->len; (to)->len++){
if (copy_RobinPrincipal(&(from)->val[(to)->len], &(to)->val[(to)->len])) return ENOMEM;
}
return 0;
}

RobinPrincipal *
add_RobinPrincipal(RobinPrincipals *data)
{
	RobinPrincipal *this;
	if (!data) return NULL;
...
int
del_RobinPrincipal(RobinPrincipals *data, RobinPrincipal *del)
{
	unsigned int u;
	RobinPrincipal *this;
