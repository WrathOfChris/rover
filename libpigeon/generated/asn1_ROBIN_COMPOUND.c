...
int
enc_ROBIN_COMPOUND(unsigned char *p, size_t len, const ROBIN_COMPOUND *data, size_t *size)
{
size_t ret = 0;
size_t l;
int i, e;
...
BACK;
e = tb_der_put_len_and_tag(p, len, ret, ASN1_C_APPL, CONS, 130, &l);
BACK;
*size = ret;
return 0;
}
...
int
dec_ROBIN_COMPOUND(const unsigned char *p, size_t len, ROBIN_COMPOUND *data, size_t *size)
{
...
reallen = 0;
e = tb_der_match_tag_and_len(p, len, ASN1_C_APPL, CONS, 130, &reallen, &l);
FORW;
{
int dce_fix = 0;
...
return 0;
fail:
free_ROBIN_COMPOUND(data);
return e;
}

void
free_ROBIN_COMPOUND(ROBIN_COMPOUND *data)
{
free_RobinHeader(&(data)->hdr);
}

size_t
len_ROBIN_COMPOUND(const ROBIN_COMPOUND *data)
{
size_t ret = 0;
{
int oldret = ret;
...
int
copy_ROBIN_COMPOUND(const ROBIN_COMPOUND *from, ROBIN_COMPOUND *to)
{
