...
int
enc_RobinDirentries(unsigned char *p, size_t len, const RobinDirentries *data, size_t *size)
{
size_t ret = 0;
size_t l;
int i, e;
...
int oldret = ret;
ret = 0;
e = enc_RobinDirentry(p, len, &(data)->val[i], &l);
BACK;
ret += oldret;
}
...
int
dec_RobinDirentries(const unsigned char *p, size_t len, RobinDirentries *data, size_t *size)
{
...
e = dec_RobinDirentry(p, len, &(data)->val[(data)->len-1], &l);
FORW;
len = origlen - ret;
}
ret += oldret;
...
return 0;
fail:
free_RobinDirentries(data);
return e;
}

void
free_RobinDirentries(RobinDirentries *data)
{
while ((data)->len){
free_RobinDirentry(&(data)->val[(data)->len-1]);
(data)->len--;
}
free((data)->val);
(data)->val = NULL;
...
size_t
len_RobinDirentries(const RobinDirentries *data)
{
size_t ret = 0;
{
int oldret = ret;
...
int oldret = ret;
ret = 0;
ret += len_RobinDirentry(&(data)->val[i]);
ret += oldret;
}
...
int
copy_RobinDirentries(const RobinDirentries *from, RobinDirentries *to)
{
if (((to)->val = malloc((from)->len * sizeof(*(to)->val))) == NULL && (from)->len != 0)
return ENOMEM;
for ((to)->len = 0; (to)->len < (from)->len; (to)->len++){
if (copy_RobinDirentry(&(from)->val[(to)->len], &(to)->val[(to)->len])) return ENOMEM;
}
return 0;
}

RobinDirentry *
add_RobinDirentry(RobinDirentries *data)
{
	RobinDirentry *this;
	if (!data) return NULL;
...
int
del_RobinDirentry(RobinDirentries *data, RobinDirentry *del)
{
	unsigned int u;
	RobinDirentry *this;
