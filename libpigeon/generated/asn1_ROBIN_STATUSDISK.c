...
int
enc_ROBIN_STATUSDISK(unsigned char *p, size_t len, const ROBIN_STATUSDISK *data, size_t *size)
{
size_t ret = 0;
size_t l;
int i, e;
...
int oldret = ret;
ret = 0;
e = enc_RobinTime(p, len, &(data)->rttlatency, &l);
BACK;
...
BACK;
e = tb_der_put_len_and_tag(p, len, ret, ASN1_C_APPL, CONS, 30, &l);
BACK;
*size = ret;
return 0;
}
...
int
dec_ROBIN_STATUSDISK(const unsigned char *p, size_t len, ROBIN_STATUSDISK *data, size_t *size)
{
...
reallen = 0;
e = tb_der_match_tag_and_len(p, len, ASN1_C_APPL, CONS, 30, &reallen, &l);
FORW;
{
int dce_fix = 0;
...
e = dec_RobinTime(p, len, &(data)->rttlatency, &l);
FORW;
if (dce_fix){
e = tb_der_match_tag_and_len(p, len, (Der_class)0, (Der_type)0, 0, &reallen, &l);
FORW;
...
return 0;
fail:
free_ROBIN_STATUSDISK(data);
return e;
}

void
free_ROBIN_STATUSDISK(ROBIN_STATUSDISK *data)
{
free_RobinStatus(&(data)->rep);
free_RobinDisk(&(data)->disk);
free_RobinTime(&(data)->rttlatency);
free_RobinDiskFlags(&(data)->flags);
}

size_t
len_ROBIN_STATUSDISK(const ROBIN_STATUSDISK *data)
{
size_t ret = 0;
{
int oldret = ret;
...
int oldret = ret;
ret = 0;
ret += len_RobinTime(&(data)->rttlatency);
ret += 1 + len_len(ret) + oldret;
}
{
int oldret = ret;
...
int
copy_ROBIN_STATUSDISK(const ROBIN_STATUSDISK *from, ROBIN_STATUSDISK *to)
{
...
if (copy_RobinTime(&(from)->rttlatency, &(to)->rttlatency)) return ENOMEM;
