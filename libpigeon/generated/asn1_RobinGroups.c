...
int
enc_RobinGroups(unsigned char *p, size_t len, const RobinGroups *data, size_t *size)
{
size_t ret = 0;
size_t l;
int i, e;
...
int oldret = ret;
ret = 0;
e = enc_RobinGroup(p, len, &(data)->val[i], &l);
BACK;
ret += oldret;
}
...
int
dec_RobinGroups(const unsigned char *p, size_t len, RobinGroups *data, size_t *size)
{
...
e = dec_RobinGroup(p, len, &(data)->val[(data)->len-1], &l);
FORW;
len = origlen - ret;
}
ret += oldret;
...
return 0;
fail:
free_RobinGroups(data);
return e;
}

void
free_RobinGroups(RobinGroups *data)
{
while ((data)->len){
free_RobinGroup(&(data)->val[(data)->len-1]);
(data)->len--;
}
free((data)->val);
(data)->val = NULL;
...
size_t
len_RobinGroups(const RobinGroups *data)
{
size_t ret = 0;
{
int oldret = ret;
...
int
copy_RobinGroups(const RobinGroups *from, RobinGroups *to)
{
if (((to)->val = malloc((from)->len * sizeof(*(to)->val))) == NULL && (from)->len != 0)
return ENOMEM;
for ((to)->len = 0; (to)->len < (from)->len; (to)->len++){
if (copy_RobinGroup(&(from)->val[(to)->len], &(to)->val[(to)->len])) return ENOMEM;
}
return 0;
}

RobinGroup *
add_RobinGroup(RobinGroups *data)
{
	RobinGroup *this;
	if (!data) return NULL;
...
int
del_RobinGroup(RobinGroups *data, RobinGroup *del)
{
	unsigned int u;
	RobinGroup *this;
