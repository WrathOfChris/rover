...
int
enc_RobinEncryptionKeys(unsigned char *p, size_t len, const RobinEncryptionKeys *data, size_t *size)
{
size_t ret = 0;
size_t l;
int i, e;
...
int oldret = ret;
ret = 0;
e = enc_RobinEncryptionKey(p, len, &(data)->val[i], &l);
BACK;
ret += oldret;
}
...
int
dec_RobinEncryptionKeys(const unsigned char *p, size_t len, RobinEncryptionKeys *data, size_t *size)
{
...
e = dec_RobinEncryptionKey(p, len, &(data)->val[(data)->len-1], &l);
FORW;
len = origlen - ret;
}
ret += oldret;
...
return 0;
fail:
free_RobinEncryptionKeys(data);
return e;
}

void
free_RobinEncryptionKeys(RobinEncryptionKeys *data)
{
while ((data)->len){
free_RobinEncryptionKey(&(data)->val[(data)->len-1]);
(data)->len--;
}
free((data)->val);
(data)->val = NULL;
...
size_t
len_RobinEncryptionKeys(const RobinEncryptionKeys *data)
{
size_t ret = 0;
{
int oldret = ret;
...
int oldret = ret;
ret = 0;
ret += len_RobinEncryptionKey(&(data)->val[i]);
ret += oldret;
}
...
int
copy_RobinEncryptionKeys(const RobinEncryptionKeys *from, RobinEncryptionKeys *to)
{
if (((to)->val = malloc((from)->len * sizeof(*(to)->val))) == NULL && (from)->len != 0)
return ENOMEM;
for ((to)->len = 0; (to)->len < (from)->len; (to)->len++){
if (copy_RobinEncryptionKey(&(from)->val[(to)->len], &(to)->val[(to)->len])) return ENOMEM;
}
return 0;
}

RobinEncryptionKey *
add_RobinEncryptionKey(RobinEncryptionKeys *data)
{
	RobinEncryptionKey *this;
	if (!data) return NULL;
...
int
del_RobinEncryptionKey(RobinEncryptionKeys *data, RobinEncryptionKey *del)
{
	unsigned int u;
	RobinEncryptionKey *this;
