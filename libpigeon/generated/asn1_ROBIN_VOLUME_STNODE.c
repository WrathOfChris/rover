...
int
enc_ROBIN_VOLUME_STNODE(unsigned char *p, size_t len, const ROBIN_VOLUME_STNODE *data, size_t *size)
{
size_t ret = 0;
size_t l;
int i, e;
...
BACK;
e = tb_der_put_len_and_tag(p, len, ret, ASN1_C_APPL, CONS, 105, &l);
BACK;
*size = ret;
return 0;
}
...
int
dec_ROBIN_VOLUME_STNODE(const unsigned char *p, size_t len, ROBIN_VOLUME_STNODE *data, size_t *size)
{
...
reallen = 0;
e = tb_der_match_tag_and_len(p, len, ASN1_C_APPL, CONS, 105, &reallen, &l);
FORW;
{
int dce_fix = 0;
...
return 0;
fail:
free_ROBIN_VOLUME_STNODE(data);
return e;
}

void
free_ROBIN_VOLUME_STNODE(ROBIN_VOLUME_STNODE *data)
{
...
size_t
len_ROBIN_VOLUME_STNODE(const ROBIN_VOLUME_STNODE *data)
{
size_t ret = 0;
{
int oldret = ret;
...
int
copy_ROBIN_VOLUME_STNODE(const ROBIN_VOLUME_STNODE *from, ROBIN_VOLUME_STNODE *to)
{
