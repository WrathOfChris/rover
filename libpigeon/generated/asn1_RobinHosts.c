...
int
enc_RobinHosts(unsigned char *p, size_t len, const RobinHosts *data, size_t *size)
{
size_t ret = 0;
size_t l;
int i, e;
...
int oldret = ret;
ret = 0;
e = enc_RobinHost(p, len, &(data)->val[i], &l);
BACK;
ret += oldret;
}
...
int
dec_RobinHosts(const unsigned char *p, size_t len, RobinHosts *data, size_t *size)
{
...
e = dec_RobinHost(p, len, &(data)->val[(data)->len-1], &l);
FORW;
len = origlen - ret;
}
ret += oldret;
...
return 0;
fail:
free_RobinHosts(data);
return e;
}

void
free_RobinHosts(RobinHosts *data)
{
while ((data)->len){
free_RobinHost(&(data)->val[(data)->len-1]);
(data)->len--;
}
free((data)->val);
(data)->val = NULL;
...
size_t
len_RobinHosts(const RobinHosts *data)
{
size_t ret = 0;
{
int oldret = ret;
...
int
copy_RobinHosts(const RobinHosts *from, RobinHosts *to)
{
if (((to)->val = malloc((from)->len * sizeof(*(to)->val))) == NULL && (from)->len != 0)
return ENOMEM;
for ((to)->len = 0; (to)->len < (from)->len; (to)->len++){
if (copy_RobinHost(&(from)->val[(to)->len], &(to)->val[(to)->len])) return ENOMEM;
}
return 0;
}

RobinHost *
add_RobinHost(RobinHosts *data)
{
	RobinHost *this;
	if (!data) return NULL;
...
int
del_RobinHost(RobinHosts *data, RobinHost *del)
{
	unsigned int u;
	RobinHost *this;
