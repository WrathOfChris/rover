...
int
enc_RobinChecksums(unsigned char *p, size_t len, const RobinChecksums *data, size_t *size)
{
size_t ret = 0;
size_t l;
int i, e;
...
int oldret = ret;
ret = 0;
e = enc_RobinChecksum(p, len, &(data)->val[i], &l);
BACK;
ret += oldret;
}
...
int
dec_RobinChecksums(const unsigned char *p, size_t len, RobinChecksums *data, size_t *size)
{
...
e = dec_RobinChecksum(p, len, &(data)->val[(data)->len-1], &l);
FORW;
len = origlen - ret;
}
ret += oldret;
...
return 0;
fail:
free_RobinChecksums(data);
return e;
}

void
free_RobinChecksums(RobinChecksums *data)
{
while ((data)->len){
free_RobinChecksum(&(data)->val[(data)->len-1]);
(data)->len--;
}
free((data)->val);
(data)->val = NULL;
...
size_t
len_RobinChecksums(const RobinChecksums *data)
{
size_t ret = 0;
{
int oldret = ret;
...
int oldret = ret;
ret = 0;
ret += len_RobinChecksum(&(data)->val[i]);
ret += oldret;
}
...
int
copy_RobinChecksums(const RobinChecksums *from, RobinChecksums *to)
{
if (((to)->val = malloc((from)->len * sizeof(*(to)->val))) == NULL && (from)->len != 0)
return ENOMEM;
for ((to)->len = 0; (to)->len < (from)->len; (to)->len++){
if (copy_RobinChecksum(&(from)->val[(to)->len], &(to)->val[(to)->len])) return ENOMEM;
}
return 0;
}

RobinChecksum *
add_RobinChecksum(RobinChecksums *data)
{
	RobinChecksum *this;
	if (!data) return NULL;
...
int
del_RobinChecksum(RobinChecksums *data, RobinChecksum *del)
{
	unsigned int u;
	RobinChecksum *this;
