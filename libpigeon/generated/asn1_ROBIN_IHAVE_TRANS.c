...
int
enc_ROBIN_IHAVE_TRANS(unsigned char *p, size_t len, const ROBIN_IHAVE_TRANS *data, size_t *size)
{
size_t ret = 0;
size_t l;
int i, e;
...
int oldret = ret;
ret = 0;
e = enc_int64(p, len, &(data)->append, &l);
BACK;
...
int oldret = ret;
ret = 0;
e = enc_RobinTransFlags(p, len, &(data)->flags, &l);
BACK;
...
BACK;
e = tb_der_put_len_and_tag(p, len, ret, ASN1_C_APPL, CONS, 96, &l);
BACK;
*size = ret;
return 0;
}
...
int
dec_ROBIN_IHAVE_TRANS(const unsigned char *p, size_t len, ROBIN_IHAVE_TRANS *data, size_t *size)
{
...
reallen = 0;
e = tb_der_match_tag_and_len(p, len, ASN1_C_APPL, CONS, 96, &reallen, &l);
FORW;
{
int dce_fix = 0;
...
e = dec_RobinTransFlags(p, len, &(data)->flags, &l);
FORW;
if (dce_fix){
e = tb_der_match_tag_and_len(p, len, (Der_class)0, (Der_type)0, 0, &reallen, &l);
FORW;
...
e = dec_int64(p, len, &(data)->append, &l);
FORW;
if (dce_fix){
e = tb_der_match_tag_and_len(p, len, (Der_class)0, (Der_type)0, 0, &reallen, &l);
FORW;
...
return 0;
fail:
free_ROBIN_IHAVE_TRANS(data);
return e;
}

void
free_ROBIN_IHAVE_TRANS(ROBIN_IHAVE_TRANS *data)
{
...
size_t
len_ROBIN_IHAVE_TRANS(const ROBIN_IHAVE_TRANS *data)
{
size_t ret = 0;
{
int oldret = ret;
...
int oldret = ret;
ret = 0;
ret += len_RobinTransFlags(&(data)->flags);
ret += 1 + len_len(ret) + oldret;
}
{
int oldret = ret;
...
int oldret = ret;
ret = 0;
ret += len_int64(&(data)->append);
ret += 1 + len_len(ret) + oldret;
}
{
int oldret = ret;
ret = 0;
ret += len_bytes(&(data)->txid);
ret += 1 + len_len(ret) + oldret;
}
ret += 1 + len_len(ret);
ret += 2 + len_len (ret);
...
int
copy_ROBIN_IHAVE_TRANS(const ROBIN_IHAVE_TRANS *from, ROBIN_IHAVE_TRANS *to)
{
