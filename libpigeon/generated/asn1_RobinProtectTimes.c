...
int
enc_RobinProtectTimes(unsigned char *p, size_t len, const RobinProtectTimes *data, size_t *size)
{
size_t ret = 0;
size_t l;
int i, e;
...
int oldret = ret;
ret = 0;
e = enc_RobinProtectTime(p, len, &(data)->val[i], &l);
BACK;
ret += oldret;
}
...
int
dec_RobinProtectTimes(const unsigned char *p, size_t len, RobinProtectTimes *data, size_t *size)
{
...
e = dec_RobinProtectTime(p, len, &(data)->val[(data)->len-1], &l);
FORW;
len = origlen - ret;
}
ret += oldret;
...
return 0;
fail:
free_RobinProtectTimes(data);
return e;
}

void
free_RobinProtectTimes(RobinProtectTimes *data)
{
while ((data)->len){
free_RobinProtectTime(&(data)->val[(data)->len-1]);
(data)->len--;
}
free((data)->val);
(data)->val = NULL;
...
size_t
len_RobinProtectTimes(const RobinProtectTimes *data)
{
size_t ret = 0;
{
int oldret = ret;
...
int oldret = ret;
ret = 0;
ret += len_RobinProtectTime(&(data)->val[i]);
ret += oldret;
}
...
int
copy_RobinProtectTimes(const RobinProtectTimes *from, RobinProtectTimes *to)
{
if (((to)->val = malloc((from)->len * sizeof(*(to)->val))) == NULL && (from)->len != 0)
return ENOMEM;
for ((to)->len = 0; (to)->len < (from)->len; (to)->len++){
if (copy_RobinProtectTime(&(from)->val[(to)->len], &(to)->val[(to)->len])) return ENOMEM;
}
return 0;
}

RobinProtectTime *
add_RobinProtectTime(RobinProtectTimes *data)
{
	RobinProtectTime *this;
	if (!data) return NULL;
...
int
del_RobinProtectTime(RobinProtectTimes *data, RobinProtectTime *del)
{
	unsigned int u;
	RobinProtectTime *this;
