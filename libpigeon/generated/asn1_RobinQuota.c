...
int
enc_RobinQuota(unsigned char *p, size_t len, const RobinQuota *data, size_t *size)
{
size_t ret = 0;
size_t l;
int i, e;
...
int oldret = ret;
ret = 0;
e = enc_int64(p, len, &(data)->nodes, &l);
BACK;
...
int oldret = ret;
ret = 0;
e = enc_int64(p, len, &(data)->space, &l);
BACK;
...
int
dec_RobinQuota(const unsigned char *p, size_t len, RobinQuota *data, size_t *size)
{
...
e = dec_int64(p, len, &(data)->space, &l);
FORW;
if (dce_fix){
...
e = dec_int64(p, len, &(data)->nodes, &l);
FORW;
if (dce_fix){
e = tb_der_match_tag_and_len(p, len, (Der_class)0, (Der_type)0, 0, &reallen, &l);
FORW;
...
return 0;
fail:
free_RobinQuota(data);
return e;
}

void
free_RobinQuota(RobinQuota *data)
{
}

size_t
len_RobinQuota(const RobinQuota *data)
{
size_t ret = 0;
{
int oldret = ret;
ret = 0;
ret += len_int64(&(data)->space);
ret += 1 + len_len(ret) + oldret;
}
{
int oldret = ret;
ret = 0;
ret += len_int64(&(data)->nodes);
ret += 1 + len_len(ret) + oldret;
}
ret += 1 + len_len(ret);
return ret;
...
int
copy_RobinQuota(const RobinQuota *from, RobinQuota *to)
{
*(&(to)->space) = *(&(from)->space);
*(&(to)->nodes) = *(&(from)->nodes);
return 0;
