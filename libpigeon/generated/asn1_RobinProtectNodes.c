...
int
enc_RobinProtectNodes(unsigned char *p, size_t len, const RobinProtectNodes *data, size_t *size)
{
size_t ret = 0;
size_t l;
int i, e;
...
int oldret = ret;
ret = 0;
e = enc_RobinProtectNode(p, len, &(data)->val[i], &l);
BACK;
ret += oldret;
}
...
int
dec_RobinProtectNodes(const unsigned char *p, size_t len, RobinProtectNodes *data, size_t *size)
{
...
e = dec_RobinProtectNode(p, len, &(data)->val[(data)->len-1], &l);
FORW;
len = origlen - ret;
}
ret += oldret;
...
return 0;
fail:
free_RobinProtectNodes(data);
return e;
}

void
free_RobinProtectNodes(RobinProtectNodes *data)
{
while ((data)->len){
free_RobinProtectNode(&(data)->val[(data)->len-1]);
(data)->len--;
}
free((data)->val);
(data)->val = NULL;
...
size_t
len_RobinProtectNodes(const RobinProtectNodes *data)
{
size_t ret = 0;
{
int oldret = ret;
...
int oldret = ret;
ret = 0;
ret += len_RobinProtectNode(&(data)->val[i]);
ret += oldret;
}
...
int
copy_RobinProtectNodes(const RobinProtectNodes *from, RobinProtectNodes *to)
{
if (((to)->val = malloc((from)->len * sizeof(*(to)->val))) == NULL && (from)->len != 0)
return ENOMEM;
for ((to)->len = 0; (to)->len < (from)->len; (to)->len++){
if (copy_RobinProtectNode(&(from)->val[(to)->len], &(to)->val[(to)->len])) return ENOMEM;
}
return 0;
}

RobinProtectNode *
add_RobinProtectNode(RobinProtectNodes *data)
{
	RobinProtectNode *this;
	if (!data) return NULL;
...
int
del_RobinProtectNode(RobinProtectNodes *data, RobinProtectNode *del)
{
	unsigned int u;
	RobinProtectNode *this;
