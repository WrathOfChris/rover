...
int
enc_RobinSyncCommands(unsigned char *p, size_t len, const RobinSyncCommands *data, size_t *size)
{
size_t ret = 0;
size_t l;
int i, e;
...
int oldret = ret;
ret = 0;
e = enc_RobinSyncCommand(p, len, &(data)->val[i], &l);
BACK;
ret += oldret;
}
...
int
dec_RobinSyncCommands(const unsigned char *p, size_t len, RobinSyncCommands *data, size_t *size)
{
...
e = dec_RobinSyncCommand(p, len, &(data)->val[(data)->len-1], &l);
FORW;
len = origlen - ret;
}
ret += oldret;
...
return 0;
fail:
free_RobinSyncCommands(data);
return e;
}

void
free_RobinSyncCommands(RobinSyncCommands *data)
{
while ((data)->len){
free_RobinSyncCommand(&(data)->val[(data)->len-1]);
(data)->len--;
}
free((data)->val);
(data)->val = NULL;
...
size_t
len_RobinSyncCommands(const RobinSyncCommands *data)
{
size_t ret = 0;
{
int oldret = ret;
...
int oldret = ret;
ret = 0;
ret += len_RobinSyncCommand(&(data)->val[i]);
ret += oldret;
}
...
int
copy_RobinSyncCommands(const RobinSyncCommands *from, RobinSyncCommands *to)
{
if (((to)->val = malloc((from)->len * sizeof(*(to)->val))) == NULL && (from)->len != 0)
return ENOMEM;
for ((to)->len = 0; (to)->len < (from)->len; (to)->len++){
if (copy_RobinSyncCommand(&(from)->val[(to)->len], &(to)->val[(to)->len])) return ENOMEM;
}
return 0;
}

RobinSyncCommand *
add_RobinSyncCommand(RobinSyncCommands *data)
{
	RobinSyncCommand *this;
	if (!data) return NULL;
...
int
del_RobinSyncCommand(RobinSyncCommands *data, RobinSyncCommand *del)
{
	unsigned int u;
	RobinSyncCommand *this;
