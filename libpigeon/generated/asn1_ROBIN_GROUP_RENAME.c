...
int
enc_ROBIN_GROUP_RENAME(unsigned char *p, size_t len, const ROBIN_GROUP_RENAME *data, size_t *size)
{
size_t ret = 0;
size_t l;
int i, e;
...
int oldret = ret;
ret = 0;
e = enc_RobinPrincipal(p, len, &(data)->new_group, &l);
BACK;
...
int oldret = ret;
ret = 0;
e = enc_RobinPrincipal(p, len, &(data)->old_group, &l);
BACK;
...
BACK;
e = tb_der_put_len_and_tag(p, len, ret, ASN1_C_APPL, CONS, 83, &l);
BACK;
*size = ret;
return 0;
}
...
int
dec_ROBIN_GROUP_RENAME(const unsigned char *p, size_t len, ROBIN_GROUP_RENAME *data, size_t *size)
{
...
reallen = 0;
e = tb_der_match_tag_and_len(p, len, ASN1_C_APPL, CONS, 83, &reallen, &l);
FORW;
{
int dce_fix = 0;
...
e = dec_RobinPrincipal(p, len, &(data)->old_group, &l);
FORW;
if (dce_fix){
e = tb_der_match_tag_and_len(p, len, (Der_class)0, (Der_type)0, 0, &reallen, &l);
FORW;
...
e = dec_RobinPrincipal(p, len, &(data)->new_group, &l);
FORW;
if (dce_fix){
e = tb_der_match_tag_and_len(p, len, (Der_class)0, (Der_type)0, 0, &reallen, &l);
FORW;
...
return 0;
fail:
free_ROBIN_GROUP_RENAME(data);
return e;
}

void
free_ROBIN_GROUP_RENAME(ROBIN_GROUP_RENAME *data)
{
free_RobinHeader(&(data)->hdr);
free_RobinPrincipal(&(data)->old_group);
free_RobinPrincipal(&(data)->new_group);
}

size_t
len_ROBIN_GROUP_RENAME(const ROBIN_GROUP_RENAME *data)
{
size_t ret = 0;
{
int oldret = ret;
...
int oldret = ret;
ret = 0;
ret += len_RobinPrincipal(&(data)->old_group);
ret += 1 + len_len(ret) + oldret;
}
{
int oldret = ret;
ret = 0;
ret += len_RobinPrincipal(&(data)->new_group);
ret += 1 + len_len(ret) + oldret;
}
ret += 1 + len_len(ret);
ret += 2 + len_len (ret);
...
int
copy_ROBIN_GROUP_RENAME(const ROBIN_GROUP_RENAME *from, ROBIN_GROUP_RENAME *to)
{
if (copy_RobinHeader(&(from)->hdr, &(to)->hdr)) return ENOMEM;
if (copy_RobinPrincipal(&(from)->old_group, &(to)->old_group)) return ENOMEM;
if (copy_RobinPrincipal(&(from)->new_group, &(to)->new_group)) return ENOMEM;
return 0;
}
