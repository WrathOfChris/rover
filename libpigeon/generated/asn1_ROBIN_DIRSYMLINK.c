...
int
enc_ROBIN_DIRSYMLINK(unsigned char *p, size_t len, const ROBIN_DIRSYMLINK *data, size_t *size)
{
size_t ret = 0;
size_t l;
int i, e;
...
int oldret = ret;
ret = 0;
e = enc_RobinFilename(p, len, &(data)->dest, &l);
BACK;
...
int oldret = ret;
ret = 0;
e = enc_RobinHandle(p, len, &(data)->phdl, &l);
BACK;
...
BACK;
e = tb_der_put_len_and_tag(p, len, ret, ASN1_C_APPL, CONS, 76, &l);
BACK;
*size = ret;
return 0;
}
...
int
dec_ROBIN_DIRSYMLINK(const unsigned char *p, size_t len, ROBIN_DIRSYMLINK *data, size_t *size)
{
...
reallen = 0;
e = tb_der_match_tag_and_len(p, len, ASN1_C_APPL, CONS, 76, &reallen, &l);
FORW;
{
int dce_fix = 0;
...
e = dec_RobinHandle(p, len, &(data)->phdl, &l);
FORW;
if (dce_fix){
e = tb_der_match_tag_and_len(p, len, (Der_class)0, (Der_type)0, 0, &reallen, &l);
FORW;
...
e = dec_RobinFilename(p, len, &(data)->dest, &l);
FORW;
if (dce_fix){
e = tb_der_match_tag_and_len(p, len, (Der_class)0, (Der_type)0, 0, &reallen, &l);
FORW;
...
return 0;
fail:
free_ROBIN_DIRSYMLINK(data);
return e;
}

void
free_ROBIN_DIRSYMLINK(ROBIN_DIRSYMLINK *data)
{
...
size_t
len_ROBIN_DIRSYMLINK(const ROBIN_DIRSYMLINK *data)
{
size_t ret = 0;
{
int oldret = ret;
...
int oldret = ret;
ret = 0;
ret += len_RobinFilename(&(data)->dest);
ret += 1 + len_len(ret) + oldret;
}
if((data)->lock){
int oldret = ret;
...
int
copy_ROBIN_DIRSYMLINK(const ROBIN_DIRSYMLINK *from, ROBIN_DIRSYMLINK *to)
{
...
(to)->vhdl = NULL;
if (copy_RobinHandle(&(from)->phdl, &(to)->phdl)) return ENOMEM;
if ((from)->hdl) {
...
if (copy_RobinFilename(&(from)->dest, &(to)->dest)) return ENOMEM;
if ((from)->lock) {
