...
int
enc_RobinTimes(unsigned char *p, size_t len, const RobinTimes *data, size_t *size)
{
size_t ret = 0;
size_t l;
int i, e;
...
int oldret = ret;
ret = 0;
e = enc_RobinTime(p, len, &(data)->val[i], &l);
BACK;
ret += oldret;
}
...
int
dec_RobinTimes(const unsigned char *p, size_t len, RobinTimes *data, size_t *size)
{
...
e = dec_RobinTime(p, len, &(data)->val[(data)->len-1], &l);
FORW;
len = origlen - ret;
}
ret += oldret;
...
return 0;
fail:
free_RobinTimes(data);
return e;
}

void
free_RobinTimes(RobinTimes *data)
{
while ((data)->len){
free_RobinTime(&(data)->val[(data)->len-1]);
(data)->len--;
}
free((data)->val);
(data)->val = NULL;
...
size_t
len_RobinTimes(const RobinTimes *data)
{
size_t ret = 0;
{
int oldret = ret;
...
int
copy_RobinTimes(const RobinTimes *from, RobinTimes *to)
{
if (((to)->val = malloc((from)->len * sizeof(*(to)->val))) == NULL && (from)->len != 0)
return ENOMEM;
for ((to)->len = 0; (to)->len < (from)->len; (to)->len++){
if (copy_RobinTime(&(from)->val[(to)->len], &(to)->val[(to)->len])) return ENOMEM;
}
return 0;
}

RobinTime *
add_RobinTime(RobinTimes *data)
{
	RobinTime *this;
	if (!data) return NULL;
...
int
del_RobinTime(RobinTimes *data, RobinTime *del)
{
	unsigned int u;
	RobinTime *this;
