...
int
enc_ROBIN_DISK_BALANCE(unsigned char *p, size_t len, const ROBIN_DISK_BALANCE *data, size_t *size)
{
size_t ret = 0;
size_t l;
int i, e;
...
int oldret = ret;
ret = 0;
e = enc_RobinDiskTarget(p, len, &(data)->target, &l);
BACK;
...
int oldret = ret;
ret = 0;
e = enc_RobinDiskFlags(p, len, (data)->flags, &l);
BACK;
...
BACK;
e = tb_der_put_len_and_tag(p, len, ret, ASN1_C_APPL, CONS, 143, &l);
BACK;
*size = ret;
return 0;
}
...
int
dec_ROBIN_DISK_BALANCE(const unsigned char *p, size_t len, ROBIN_DISK_BALANCE *data, size_t *size)
{
...
reallen = 0;
e = tb_der_match_tag_and_len(p, len, ASN1_C_APPL, CONS, 143, &reallen, &l);
FORW;
{
int dce_fix = 0;
...
e = dec_RobinDiskFlags(p, len, (data)->flags, &l);
FORW;
if (dce_fix){
e = tb_der_match_tag_and_len(p, len, (Der_class)0, (Der_type)0, 0, &reallen, &l);
FORW;
...
e = dec_RobinDiskTarget(p, len, &(data)->target, &l);
FORW;
if (dce_fix){
e = tb_der_match_tag_and_len(p, len, (Der_class)0, (Der_type)0, 0, &reallen, &l);
FORW;
...
return 0;
fail:
free_ROBIN_DISK_BALANCE(data);
return e;
}

void
free_ROBIN_DISK_BALANCE(ROBIN_DISK_BALANCE *data)
{
free_RobinHeader(&(data)->hdr);
free_RobinDisk(&(data)->disk);
if ((data)->flags) {
free_RobinDiskFlags((data)->flags);
free((data)->flags);
(data)->flags = NULL;
}
free_RobinDiskTarget(&(data)->target);
}

size_t
len_ROBIN_DISK_BALANCE(const ROBIN_DISK_BALANCE *data)
{
size_t ret = 0;
{
int oldret = ret;
...
int oldret = ret;
ret = 0;
ret += len_RobinDiskFlags((data)->flags);
ret += 1 + len_len(ret) + oldret;
}
{
int oldret = ret;
ret = 0;
ret += len_RobinDiskTarget(&(data)->target);
ret += 1 + len_len(ret) + oldret;
}
ret += 1 + len_len(ret);
ret += 3 + len_len (ret);
...
int
copy_ROBIN_DISK_BALANCE(const ROBIN_DISK_BALANCE *from, ROBIN_DISK_BALANCE *to)
{
...
if (copy_RobinDiskFlags((from)->flags, (to)->flags)) return ENOMEM;
} else
(to)->flags = NULL;
if (copy_RobinDiskTarget(&(from)->target, &(to)->target)) return ENOMEM;
return 0;
}
