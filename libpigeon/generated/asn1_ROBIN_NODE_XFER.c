...
int
enc_ROBIN_NODE_XFER(unsigned char *p, size_t len, const ROBIN_NODE_XFER *data, size_t *size)
{
size_t ret = 0;
size_t l;
int i, e;
...
BACK;
e = tb_der_put_len_and_tag(p, len, ret, ASN1_C_APPL, CONS, 64, &l);
BACK;
*size = ret;
return 0;
}
...
int
dec_ROBIN_NODE_XFER(const unsigned char *p, size_t len, ROBIN_NODE_XFER *data, size_t *size)
{
...
reallen = 0;
e = tb_der_match_tag_and_len(p, len, ASN1_C_APPL, CONS, 64, &reallen, &l);
FORW;
{
int dce_fix = 0;
...
return 0;
fail:
free_ROBIN_NODE_XFER(data);
return e;
}

void
free_ROBIN_NODE_XFER(ROBIN_NODE_XFER *data)
{
...
size_t
len_ROBIN_NODE_XFER(const ROBIN_NODE_XFER *data)
{
size_t ret = 0;
{
int oldret = ret;
...
int
copy_ROBIN_NODE_XFER(const ROBIN_NODE_XFER *from, ROBIN_NODE_XFER *to)
{
