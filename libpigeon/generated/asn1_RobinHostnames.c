...
int
enc_RobinHostnames(unsigned char *p, size_t len, const RobinHostnames *data, size_t *size)
{
size_t ret = 0;
size_t l;
int i, e;
...
int oldret = ret;
ret = 0;
e = enc_RobinHostname(p, len, &(data)->val[i], &l);
BACK;
ret += oldret;
}
...
int
dec_RobinHostnames(const unsigned char *p, size_t len, RobinHostnames *data, size_t *size)
{
...
e = dec_RobinHostname(p, len, &(data)->val[(data)->len-1], &l);
FORW;
len = origlen - ret;
}
ret += oldret;
...
return 0;
fail:
free_RobinHostnames(data);
return e;
}

void
free_RobinHostnames(RobinHostnames *data)
{
while ((data)->len){
free_RobinHostname(&(data)->val[(data)->len-1]);
(data)->len--;
}
free((data)->val);
(data)->val = NULL;
...
size_t
len_RobinHostnames(const RobinHostnames *data)
{
size_t ret = 0;
{
int oldret = ret;
...
int oldret = ret;
ret = 0;
ret += len_RobinHostname(&(data)->val[i]);
ret += oldret;
}
...
int
copy_RobinHostnames(const RobinHostnames *from, RobinHostnames *to)
{
if (((to)->val = malloc((from)->len * sizeof(*(to)->val))) == NULL && (from)->len != 0)
return ENOMEM;
for ((to)->len = 0; (to)->len < (from)->len; (to)->len++){
if (copy_RobinHostname(&(from)->val[(to)->len], &(to)->val[(to)->len])) return ENOMEM;
}
return 0;
}

RobinHostname *
add_RobinHostname(RobinHostnames *data)
{
	RobinHostname *this;
	if (!data) return NULL;
...
int
del_RobinHostname(RobinHostnames *data, RobinHostname *del)
{
	unsigned int u;
	RobinHostname *this;
