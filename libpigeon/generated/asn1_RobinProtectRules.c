...
int
enc_RobinProtectRules(unsigned char *p, size_t len, const RobinProtectRules *data, size_t *size)
{
size_t ret = 0;
size_t l;
int i, e;
...
int oldret = ret;
ret = 0;
e = enc_RobinProtectRule(p, len, &(data)->val[i], &l);
BACK;
ret += oldret;
}
...
int
dec_RobinProtectRules(const unsigned char *p, size_t len, RobinProtectRules *data, size_t *size)
{
...
e = dec_RobinProtectRule(p, len, &(data)->val[(data)->len-1], &l);
FORW;
len = origlen - ret;
}
ret += oldret;
...
return 0;
fail:
free_RobinProtectRules(data);
return e;
}

void
free_RobinProtectRules(RobinProtectRules *data)
{
while ((data)->len){
free_RobinProtectRule(&(data)->val[(data)->len-1]);
(data)->len--;
}
free((data)->val);
(data)->val = NULL;
...
size_t
len_RobinProtectRules(const RobinProtectRules *data)
{
size_t ret = 0;
{
int oldret = ret;
...
int oldret = ret;
ret = 0;
ret += len_RobinProtectRule(&(data)->val[i]);
ret += oldret;
}
...
int
copy_RobinProtectRules(const RobinProtectRules *from, RobinProtectRules *to)
{
if (((to)->val = malloc((from)->len * sizeof(*(to)->val))) == NULL && (from)->len != 0)
return ENOMEM;
for ((to)->len = 0; (to)->len < (from)->len; (to)->len++){
if (copy_RobinProtectRule(&(from)->val[(to)->len], &(to)->val[(to)->len])) return ENOMEM;
}
return 0;
}

RobinProtectRule *
add_RobinProtectRule(RobinProtectRules *data)
{
	RobinProtectRule *this;
	if (!data) return NULL;
...
int
del_RobinProtectRule(RobinProtectRules *data, RobinProtectRule *del)
{
	unsigned int u;
	RobinProtectRule *this;
