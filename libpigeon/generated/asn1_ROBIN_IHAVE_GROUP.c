...
int
enc_ROBIN_IHAVE_GROUP(unsigned char *p, size_t len, const ROBIN_IHAVE_GROUP *data, size_t *size)
{
size_t ret = 0;
size_t l;
int i, e;
...
int oldret = ret;
ret = 0;
e = enc_RobinGroupFlags(p, len, &(data)->flags, &l);
BACK;
...
BACK;
e = tb_der_put_len_and_tag(p, len, ret, ASN1_C_APPL, CONS, 95, &l);
BACK;
*size = ret;
return 0;
}
...
int
dec_ROBIN_IHAVE_GROUP(const unsigned char *p, size_t len, ROBIN_IHAVE_GROUP *data, size_t *size)
{
...
reallen = 0;
e = tb_der_match_tag_and_len(p, len, ASN1_C_APPL, CONS, 95, &reallen, &l);
FORW;
{
int dce_fix = 0;
...
e = dec_RobinGroupFlags(p, len, &(data)->flags, &l);
FORW;
if (dce_fix){
e = tb_der_match_tag_and_len(p, len, (Der_class)0, (Der_type)0, 0, &reallen, &l);
FORW;
...
return 0;
fail:
free_ROBIN_IHAVE_GROUP(data);
return e;
}

void
free_ROBIN_IHAVE_GROUP(ROBIN_IHAVE_GROUP *data)
{
free_RobinHeader(&(data)->hdr);
free_RobinGroupFlags(&(data)->flags);
}

size_t
len_ROBIN_IHAVE_GROUP(const ROBIN_IHAVE_GROUP *data)
{
size_t ret = 0;
{
int oldret = ret;
...
int oldret = ret;
ret = 0;
ret += len_RobinGroupFlags(&(data)->flags);
ret += 1 + len_len(ret) + oldret;
}
ret += 1 + len_len(ret);
ret += 2 + len_len (ret);
...
int
copy_ROBIN_IHAVE_GROUP(const ROBIN_IHAVE_GROUP *from, ROBIN_IHAVE_GROUP *to)
{
if (copy_RobinHeader(&(from)->hdr, &(to)->hdr)) return ENOMEM;
if (copy_RobinGroupFlags(&(from)->flags, &(to)->flags)) return ENOMEM;
return 0;
}
