...
int
enc_RobinSignatures(unsigned char *p, size_t len, const RobinSignatures *data, size_t *size)
{
size_t ret = 0;
size_t l;
int i, e;
...
int oldret = ret;
ret = 0;
e = enc_RobinSignature(p, len, &(data)->val[i], &l);
BACK;
ret += oldret;
}
...
int
dec_RobinSignatures(const unsigned char *p, size_t len, RobinSignatures *data, size_t *size)
{
...
e = dec_RobinSignature(p, len, &(data)->val[(data)->len-1], &l);
FORW;
len = origlen - ret;
}
ret += oldret;
...
return 0;
fail:
free_RobinSignatures(data);
return e;
}

void
free_RobinSignatures(RobinSignatures *data)
{
while ((data)->len){
free_RobinSignature(&(data)->val[(data)->len-1]);
(data)->len--;
}
free((data)->val);
(data)->val = NULL;
...
size_t
len_RobinSignatures(const RobinSignatures *data)
{
size_t ret = 0;
{
int oldret = ret;
...
int oldret = ret;
ret = 0;
ret += len_RobinSignature(&(data)->val[i]);
ret += oldret;
}
...
int
copy_RobinSignatures(const RobinSignatures *from, RobinSignatures *to)
{
if (((to)->val = malloc((from)->len * sizeof(*(to)->val))) == NULL && (from)->len != 0)
return ENOMEM;
for ((to)->len = 0; (to)->len < (from)->len; (to)->len++){
if (copy_RobinSignature(&(from)->val[(to)->len], &(to)->val[(to)->len])) return ENOMEM;
}
return 0;
}

RobinSignature *
add_RobinSignature(RobinSignatures *data)
{
	RobinSignature *this;
	if (!data) return NULL;
...
int
del_RobinSignature(RobinSignatures *data, RobinSignature *del)
{
	unsigned int u;
	RobinSignature *this;
