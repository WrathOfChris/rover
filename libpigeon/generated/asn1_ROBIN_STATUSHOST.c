...
int
enc_ROBIN_STATUSHOST(unsigned char *p, size_t len, const ROBIN_STATUSHOST *data, size_t *size)
{
size_t ret = 0;
size_t l;
int i, e;
...
int oldret = ret;
ret = 0;
e = enc_RobinHost(p, len, &(data)->host, &l);
BACK;
...
BACK;
e = tb_der_put_len_and_tag(p, len, ret, ASN1_C_APPL, CONS, 31, &l);
BACK;
*size = ret;
return 0;
}
...
int
dec_ROBIN_STATUSHOST(const unsigned char *p, size_t len, ROBIN_STATUSHOST *data, size_t *size)
{
...
reallen = 0;
e = tb_der_match_tag_and_len(p, len, ASN1_C_APPL, CONS, 31, &reallen, &l);
FORW;
{
int dce_fix = 0;
...
e = dec_RobinHost(p, len, &(data)->host, &l);
FORW;
if (dce_fix){
e = tb_der_match_tag_and_len(p, len, (Der_class)0, (Der_type)0, 0, &reallen, &l);
FORW;
...
return 0;
fail:
free_ROBIN_STATUSHOST(data);
return e;
}

void
free_ROBIN_STATUSHOST(ROBIN_STATUSHOST *data)
{
free_RobinStatus(&(data)->rep);
free_RobinHost(&(data)->host);
free_bytes(&(data)->clientid);
free_RobinPrincipals(&(data)->principals);
}

size_t
len_ROBIN_STATUSHOST(const ROBIN_STATUSHOST *data)
{
size_t ret = 0;
{
int oldret = ret;
...
int
copy_ROBIN_STATUSHOST(const ROBIN_STATUSHOST *from, ROBIN_STATUSHOST *to)
{
if (copy_RobinStatus(&(from)->rep, &(to)->rep)) return ENOMEM;
if (copy_RobinHost(&(from)->host, &(to)->host)) return ENOMEM;
