...
int
enc_ROBIN_STATUSSYNC(unsigned char *p, size_t len, const ROBIN_STATUSSYNC *data, size_t *size)
{
size_t ret = 0;
size_t l;
int i, e;
...
int oldret = ret;
ret = 0;
e = enc_RobinSyncChecksum(p, len, (data)->syncsum, &l);
BACK;
...
BACK;
e = tb_der_put_len_and_tag(p, len, ret, ASN1_C_APPL, CONS, 35, &l);
BACK;
*size = ret;
return 0;
}
...
int
dec_ROBIN_STATUSSYNC(const unsigned char *p, size_t len, ROBIN_STATUSSYNC *data, size_t *size)
{
...
reallen = 0;
e = tb_der_match_tag_and_len(p, len, ASN1_C_APPL, CONS, 35, &reallen, &l);
FORW;
{
int dce_fix = 0;
...
(data)->syncsum = malloc(sizeof(*(data)->syncsum));
if ((data)->syncsum == NULL) return ENOMEM;
e = dec_RobinSyncChecksum(p, len, (data)->syncsum, &l);
FORW;
if (dce_fix){
e = tb_der_match_tag_and_len(p, len, (Der_class)0, (Der_type)0, 0, &reallen, &l);
FORW;
...
return 0;
fail:
free_ROBIN_STATUSSYNC(data);
return e;
}

void
free_ROBIN_STATUSSYNC(ROBIN_STATUSSYNC *data)
{
free_RobinStatus(&(data)->rep);
if ((data)->syncsum) {
free_RobinSyncChecksum((data)->syncsum);
free((data)->syncsum);
(data)->syncsum = NULL;
}
}
...
size_t
len_ROBIN_STATUSSYNC(const ROBIN_STATUSSYNC *data)
{
size_t ret = 0;
{
int oldret = ret;
...
int oldret = ret;
ret = 0;
ret += len_RobinSyncChecksum((data)->syncsum);
ret += 1 + len_len(ret) + oldret;
}
ret += 1 + len_len(ret);
ret += 2 + len_len (ret);
...
int
copy_ROBIN_STATUSSYNC(const ROBIN_STATUSSYNC *from, ROBIN_STATUSSYNC *to)
{
if (copy_RobinStatus(&(from)->rep, &(to)->rep)) return ENOMEM;
if ((from)->syncsum) {
(to)->syncsum = malloc(sizeof(*(to)->syncsum));
if ((to)->syncsum == NULL) return ENOMEM;
if (copy_RobinSyncChecksum((from)->syncsum, (to)->syncsum)) return ENOMEM;
} else
(to)->syncsum = NULL;
return 0;
}
