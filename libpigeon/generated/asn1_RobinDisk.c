...
int
enc_RobinDisk(unsigned char *p, size_t len, const RobinDisk *data, size_t *size)
{
size_t ret = 0;
size_t l;
int i, e;
...
int oldret = ret;
ret = 0;
e = enc_RobinPartition(p, len, &(data)->pid, &l);
BACK;
...
int
dec_RobinDisk(const unsigned char *p, size_t len, RobinDisk *data, size_t *size)
{
...
e = dec_RobinPartition(p, len, &(data)->pid, &l);
FORW;
if (dce_fix){
e = tb_der_match_tag_and_len(p, len, (Der_class)0, (Der_type)0, 0, &reallen, &l);
FORW;
...
void
free_RobinDisk(RobinDisk *data)
{
free_RobinHostname(&(data)->hostname);
free_RobinPartition(&(data)->pid);
}

size_t
len_RobinDisk(const RobinDisk *data)
{
size_t ret = 0;
{
int oldret = ret;
...
int oldret = ret;
ret = 0;
ret += len_RobinPartition(&(data)->pid);
ret += 1 + len_len(ret) + oldret;
}
ret += 1 + len_len(ret);
return ret;
...
int
copy_RobinDisk(const RobinDisk *from, RobinDisk *to)
{
if (copy_RobinHostname(&(from)->hostname, &(to)->hostname)) return ENOMEM;
if (copy_RobinPartition(&(from)->pid, &(to)->pid)) return ENOMEM;
return 0;
}
