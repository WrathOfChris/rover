...
int
enc_RobinStatus(unsigned char *p, size_t len, const RobinStatus *data, size_t *size)
{
size_t ret = 0;
size_t l;
int i, e;
...
int oldret = ret;
ret = 0;
e = enc_ROBIN_UNSIGNED(p, len, &(data)->rxid, &l);
BACK;
...
int oldret = ret;
ret = 0;
e = enc_int32(p, len, &(data)->status, &l);
BACK;
...
int
dec_RobinStatus(const unsigned char *p, size_t len, RobinStatus *data, size_t *size)
{
...
e = dec_int32(p, len, &(data)->status, &l);
FORW;
if (dce_fix){
e = tb_der_match_tag_and_len(p, len, (Der_class)0, (Der_type)0, 0, &reallen, &l);
FORW;
...
e = dec_ROBIN_UNSIGNED(p, len, &(data)->rxid, &l);
FORW;
if (dce_fix){
e = tb_der_match_tag_and_len(p, len, (Der_class)0, (Der_type)0, 0, &reallen, &l);
FORW;
...
return 0;
fail:
free_RobinStatus(data);
return e;
}

void
free_RobinStatus(RobinStatus *data)
{
free_RobinHeader(&(data)->hdr);
free_ROBIN_UNSIGNED(&(data)->rxid);
}

size_t
len_RobinStatus(const RobinStatus *data)
{
size_t ret = 0;
{
int oldret = ret;
...
int oldret = ret;
ret = 0;
ret += len_int32(&(data)->status);
ret += 1 + len_len(ret) + oldret;
}
{
int oldret = ret;
ret = 0;
ret += len_ROBIN_UNSIGNED(&(data)->rxid);
ret += 1 + len_len(ret) + oldret;
}
ret += 1 + len_len(ret);
return ret;
...
int
copy_RobinStatus(const RobinStatus *from, RobinStatus *to)
{
if (copy_RobinHeader(&(from)->hdr, &(to)->hdr)) return ENOMEM;
*(&(to)->status) = *(&(from)->status);
if (copy_ROBIN_UNSIGNED(&(from)->rxid, &(to)->rxid)) return ENOMEM;
return 0;
}
