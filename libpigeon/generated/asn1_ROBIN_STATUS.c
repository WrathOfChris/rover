...
int
enc_ROBIN_STATUS(unsigned char *p, size_t len, const ROBIN_STATUS *data, size_t *size)
{
size_t ret = 0;
size_t l;
int i, e;
...
BACK;
e = tb_der_put_len_and_tag(p, len, ret, ASN1_C_APPL, CONS, 20, &l);
BACK;
*size = ret;
return 0;
}
...
int
dec_ROBIN_STATUS(const unsigned char *p, size_t len, ROBIN_STATUS *data, size_t *size)
{
...
reallen = 0;
e = tb_der_match_tag_and_len(p, len, ASN1_C_APPL, CONS, 20, &reallen, &l);
FORW;
{
int dce_fix = 0;
...
return 0;
fail:
free_ROBIN_STATUS(data);
return e;
}

void
free_ROBIN_STATUS(ROBIN_STATUS *data)
{
free_RobinStatus(&(data)->rep);
}

size_t
len_ROBIN_STATUS(const ROBIN_STATUS *data)
{
size_t ret = 0;
{
int oldret = ret;
...
int
copy_ROBIN_STATUS(const ROBIN_STATUS *from, ROBIN_STATUS *to)
{
