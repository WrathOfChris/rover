...
int
enc_RobinTime(unsigned char *p, size_t len, const RobinTime *data, size_t *size)
{
size_t ret = 0;
size_t l;
int i, e;
...
int
dec_RobinTime(const unsigned char *p, size_t len, RobinTime *data, size_t *size)
{
...
void
free_RobinTime(RobinTime *data)
{
}

size_t
len_RobinTime(const RobinTime *data)
{
size_t ret = 0;
{
int oldret = ret;
ret = 0;
ret += len_int64(&(data)->secs);
ret += 1 + len_len(ret) + oldret;
}
{
int oldret = ret;
ret = 0;
ret += len_int32(&(data)->nsec);
ret += 1 + len_len(ret) + oldret;
}
ret += 1 + len_len(ret);
return ret;
...
int
copy_RobinTime(const RobinTime *from, RobinTime *to)
{
*(&(to)->secs) = *(&(from)->secs);
*(&(to)->nsec) = *(&(from)->nsec);
return 0;
