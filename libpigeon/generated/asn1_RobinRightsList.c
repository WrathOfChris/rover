...
int
enc_RobinRightsList(unsigned char *p, size_t len, const RobinRightsList *data, size_t *size)
{
size_t ret = 0;
size_t l;
int i, e;
...
int oldret = ret;
ret = 0;
e = enc_RobinRights(p, len, &(data)->val[i], &l);
BACK;
ret += oldret;
}
...
int
dec_RobinRightsList(const unsigned char *p, size_t len, RobinRightsList *data, size_t *size)
{
...
e = dec_RobinRights(p, len, &(data)->val[(data)->len-1], &l);
FORW;
len = origlen - ret;
}
ret += oldret;
...
return 0;
fail:
free_RobinRightsList(data);
return e;
}

void
free_RobinRightsList(RobinRightsList *data)
{
while ((data)->len){
free_RobinRights(&(data)->val[(data)->len-1]);
(data)->len--;
}
free((data)->val);
(data)->val = NULL;
...
size_t
len_RobinRightsList(const RobinRightsList *data)
{
size_t ret = 0;
{
int oldret = ret;
...
int oldret = ret;
ret = 0;
ret += len_RobinRights(&(data)->val[i]);
ret += oldret;
}
...
int
copy_RobinRightsList(const RobinRightsList *from, RobinRightsList *to)
{
if (((to)->val = malloc((from)->len * sizeof(*(to)->val))) == NULL && (from)->len != 0)
return ENOMEM;
for ((to)->len = 0; (to)->len < (from)->len; (to)->len++){
if (copy_RobinRights(&(from)->val[(to)->len], &(to)->val[(to)->len])) return ENOMEM;
}
return 0;
}

RobinRights *
add_RobinRights(RobinRightsList *data)
{
	RobinRights *this;
	if (!data) return NULL;
...
int
del_RobinRights(RobinRightsList *data, RobinRights *del)
{
	unsigned int u;
	RobinRights *this;
