...
int
enc_RobinEncryptionKey(unsigned char *p, size_t len, const RobinEncryptionKey *data, size_t *size)
{
size_t ret = 0;
size_t l;
int i, e;
...
int oldret = ret;
ret = 0;
e = enc_bytes(p, len, &(data)->keyvalue, &l);
BACK;
...
int
dec_RobinEncryptionKey(const unsigned char *p, size_t len, RobinEncryptionKey *data, size_t *size)
{
...
e = dec_bytes(p, len, &(data)->keyvalue, &l);
FORW;
if (dce_fix){
e = tb_der_match_tag_and_len(p, len, (Der_class)0, (Der_type)0, 0, &reallen, &l);
FORW;
...
return 0;
fail:
free_RobinEncryptionKey(data);
return e;
}

void
free_RobinEncryptionKey(RobinEncryptionKey *data)
{
free_bytes(&(data)->keyvalue);
}

size_t
len_RobinEncryptionKey(const RobinEncryptionKey *data)
{
size_t ret = 0;
{
int oldret = ret;
...
int
copy_RobinEncryptionKey(const RobinEncryptionKey *from, RobinEncryptionKey *to)
{
*(&(to)->keytype) = *(&(from)->keytype);
if (copy_bytes(&(from)->keyvalue, &(to)->keyvalue)) return ENOMEM;
return 0;
}
