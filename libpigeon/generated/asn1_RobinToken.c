...
int
enc_RobinToken(unsigned char *p, size_t len, const RobinToken *data, size_t *size)
{
size_t ret = 0;
size_t l;
int i, e;
...
int oldret = ret;
ret = 0;
e = enc_RobinSignatures(p, len, &(data)->sigs, &l);
BACK;
...
int oldret = ret;
ret = 0;
e = enc_RobinTokenData(p, len, &(data)->data, &l);
BACK;
...
int
dec_RobinToken(const unsigned char *p, size_t len, RobinToken *data, size_t *size)
{
...
e = dec_RobinTokenData(p, len, &(data)->data, &l);
FORW;
if (dce_fix){
...
e = dec_RobinSignatures(p, len, &(data)->sigs, &l);
FORW;
if (dce_fix){
e = tb_der_match_tag_and_len(p, len, (Der_class)0, (Der_type)0, 0, &reallen, &l);
FORW;
...
return 0;
fail:
free_RobinToken(data);
return e;
}

void
free_RobinToken(RobinToken *data)
{
free_RobinTokenData(&(data)->data);
free_RobinSignatures(&(data)->sigs);
}

size_t
len_RobinToken(const RobinToken *data)
{
size_t ret = 0;
{
int oldret = ret;
ret = 0;
ret += len_RobinTokenData(&(data)->data);
ret += 1 + len_len(ret) + oldret;
}
{
int oldret = ret;
ret = 0;
ret += len_RobinSignatures(&(data)->sigs);
ret += 1 + len_len(ret) + oldret;
}
ret += 1 + len_len(ret);
return ret;
...
int
copy_RobinToken(const RobinToken *from, RobinToken *to)
{
if (copy_RobinTokenData(&(from)->data, &(to)->data)) return ENOMEM;
if (copy_RobinSignatures(&(from)->sigs, &(to)->sigs)) return ENOMEM;
return 0;
}
