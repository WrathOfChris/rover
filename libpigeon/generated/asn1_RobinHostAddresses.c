...
int
enc_RobinHostAddresses(unsigned char *p, size_t len, const RobinHostAddresses *data, size_t *size)
{
size_t ret = 0;
size_t l;
int i, e;
...
int oldret = ret;
ret = 0;
e = enc_RobinHostAddress(p, len, &(data)->val[i], &l);
BACK;
ret += oldret;
}
...
int
dec_RobinHostAddresses(const unsigned char *p, size_t len, RobinHostAddresses *data, size_t *size)
{
...
e = dec_RobinHostAddress(p, len, &(data)->val[(data)->len-1], &l);
FORW;
len = origlen - ret;
}
ret += oldret;
...
return 0;
fail:
free_RobinHostAddresses(data);
return e;
}

void
free_RobinHostAddresses(RobinHostAddresses *data)
{
while ((data)->len){
free_RobinHostAddress(&(data)->val[(data)->len-1]);
(data)->len--;
}
free((data)->val);
(data)->val = NULL;
...
size_t
len_RobinHostAddresses(const RobinHostAddresses *data)
{
size_t ret = 0;
{
int oldret = ret;
...
int oldret = ret;
ret = 0;
ret += len_RobinHostAddress(&(data)->val[i]);
ret += oldret;
}
...
int
copy_RobinHostAddresses(const RobinHostAddresses *from, RobinHostAddresses *to)
{
if (((to)->val = malloc((from)->len * sizeof(*(to)->val))) == NULL && (from)->len != 0)
return ENOMEM;
for ((to)->len = 0; (to)->len < (from)->len; (to)->len++){
if (copy_RobinHostAddress(&(from)->val[(to)->len], &(to)->val[(to)->len])) return ENOMEM;
}
return 0;
}

RobinHostAddress *
add_RobinHostAddress(RobinHostAddresses *data)
{
	RobinHostAddress *this;
	if (!data) return NULL;
...
int
del_RobinHostAddress(RobinHostAddresses *data, RobinHostAddress *del)
{
	unsigned int u;
	RobinHostAddress *this;
