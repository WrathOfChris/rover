...
int
enc_RobinHost(unsigned char *p, size_t len, const RobinHost *data, size_t *size)
{
size_t ret = 0;
size_t l;
int i, e;
...
int oldret = ret;
ret = 0;
e = enc_RobinHostAddresses(p, len, &(data)->addresses, &l);
BACK;
...
int oldret = ret;
ret = 0;
e = enc_RobinHostname(p, len, &(data)->hostname, &l);
BACK;
...
int
dec_RobinHost(const unsigned char *p, size_t len, RobinHost *data, size_t *size)
{
...
e = dec_RobinHostname(p, len, &(data)->hostname, &l);
FORW;
if (dce_fix){
...
e = dec_RobinHostAddresses(p, len, &(data)->addresses, &l);
FORW;
if (dce_fix){
e = tb_der_match_tag_and_len(p, len, (Der_class)0, (Der_type)0, 0, &reallen, &l);
FORW;
...
void
free_RobinHost(RobinHost *data)
{
free_RobinHostname(&(data)->hostname);
free_RobinHostAddresses(&(data)->addresses);
}

size_t
len_RobinHost(const RobinHost *data)
{
size_t ret = 0;
{
int oldret = ret;
ret = 0;
ret += len_RobinHostname(&(data)->hostname);
ret += 1 + len_len(ret) + oldret;
}
{
int oldret = ret;
ret = 0;
ret += len_RobinHostAddresses(&(data)->addresses);
ret += 1 + len_len(ret) + oldret;
}
ret += 1 + len_len(ret);
return ret;
...
int
copy_RobinHost(const RobinHost *from, RobinHost *to)
{
if (copy_RobinHostname(&(from)->hostname, &(to)->hostname)) return ENOMEM;
if (copy_RobinHostAddresses(&(from)->addresses, &(to)->addresses)) return ENOMEM;
return 0;
}
