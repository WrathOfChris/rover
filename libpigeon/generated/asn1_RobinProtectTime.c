...
int
enc_RobinProtectTime(unsigned char *p, size_t len, const RobinProtectTime *data, size_t *size)
{
size_t ret = 0;
size_t l;
int i, e;
...
int oldret = ret;
ret = 0;
e = enc_RobinDateSpec(p, len, &(data)->stop, &l);
BACK;
...
int oldret = ret;
ret = 0;
e = enc_RobinDateSpec(p, len, &(data)->start, &l);
BACK;
...
int
dec_RobinProtectTime(const unsigned char *p, size_t len, RobinProtectTime *data, size_t *size)
{
...
e = dec_RobinDateSpec(p, len, &(data)->start, &l);
FORW;
if (dce_fix){
...
e = dec_RobinDateSpec(p, len, &(data)->stop, &l);
FORW;
if (dce_fix){
e = tb_der_match_tag_and_len(p, len, (Der_class)0, (Der_type)0, 0, &reallen, &l);
FORW;
...
return 0;
fail:
free_RobinProtectTime(data);
return e;
}

void
free_RobinProtectTime(RobinProtectTime *data)
{
free_RobinDateSpec(&(data)->start);
free_RobinDateSpec(&(data)->stop);
}

size_t
len_RobinProtectTime(const RobinProtectTime *data)
{
size_t ret = 0;
{
int oldret = ret;
ret = 0;
ret += len_RobinDateSpec(&(data)->start);
ret += 1 + len_len(ret) + oldret;
}
{
int oldret = ret;
ret = 0;
ret += len_RobinDateSpec(&(data)->stop);
ret += 1 + len_len(ret) + oldret;
}
ret += 1 + len_len(ret);
return ret;
...
int
copy_RobinProtectTime(const RobinProtectTime *from, RobinProtectTime *to)
{
if (copy_RobinDateSpec(&(from)->start, &(to)->start)) return ENOMEM;
if (copy_RobinDateSpec(&(from)->stop, &(to)->stop)) return ENOMEM;
return 0;
}
