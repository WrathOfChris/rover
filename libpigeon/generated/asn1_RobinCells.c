...
int
enc_RobinCells(unsigned char *p, size_t len, const RobinCells *data, size_t *size)
{
size_t ret = 0;
size_t l;
int i, e;
...
int oldret = ret;
ret = 0;
e = enc_RobinCell(p, len, &(data)->val[i], &l);
BACK;
ret += oldret;
}
...
int
dec_RobinCells(const unsigned char *p, size_t len, RobinCells *data, size_t *size)
{
...
e = dec_RobinCell(p, len, &(data)->val[(data)->len-1], &l);
FORW;
len = origlen - ret;
}
ret += oldret;
...
return 0;
fail:
free_RobinCells(data);
return e;
}

void
free_RobinCells(RobinCells *data)
{
while ((data)->len){
free_RobinCell(&(data)->val[(data)->len-1]);
(data)->len--;
}
free((data)->val);
(data)->val = NULL;
...
size_t
len_RobinCells(const RobinCells *data)
{
size_t ret = 0;
{
int oldret = ret;
...
int
copy_RobinCells(const RobinCells *from, RobinCells *to)
{
if (((to)->val = malloc((from)->len * sizeof(*(to)->val))) == NULL && (from)->len != 0)
return ENOMEM;
for ((to)->len = 0; (to)->len < (from)->len; (to)->len++){
if (copy_RobinCell(&(from)->val[(to)->len], &(to)->val[(to)->len])) return ENOMEM;
}
return 0;
}

RobinCell *
add_RobinCell(RobinCells *data)
{
	RobinCell *this;
	if (!data) return NULL;
...
int
del_RobinCell(RobinCells *data, RobinCell *del)
{
	unsigned int u;
	RobinCell *this;
