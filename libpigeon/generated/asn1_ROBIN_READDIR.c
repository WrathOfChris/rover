...
int
enc_ROBIN_READDIR(unsigned char *p, size_t len, const ROBIN_READDIR *data, size_t *size)
{
size_t ret = 0;
size_t l;
int i, e;
...
int oldret = ret;
ret = 0;
e = enc_RobinDirentry(p, len, (data)->last, &l);
BACK;
...
BACK;
e = tb_der_put_len_and_tag(p, len, ret, ASN1_C_APPL, CONS, 71, &l);
BACK;
*size = ret;
return 0;
}
...
int
dec_ROBIN_READDIR(const unsigned char *p, size_t len, ROBIN_READDIR *data, size_t *size)
{
...
reallen = 0;
e = tb_der_match_tag_and_len(p, len, ASN1_C_APPL, CONS, 71, &reallen, &l);
FORW;
{
int dce_fix = 0;
...
(data)->last = malloc(sizeof(*(data)->last));
if ((data)->last == NULL) return ENOMEM;
e = dec_RobinDirentry(p, len, (data)->last, &l);
FORW;
if (dce_fix){
e = tb_der_match_tag_and_len(p, len, (Der_class)0, (Der_type)0, 0, &reallen, &l);
FORW;
...
return 0;
fail:
free_ROBIN_READDIR(data);
return e;
}

void
free_ROBIN_READDIR(ROBIN_READDIR *data)
{
...
size_t
len_ROBIN_READDIR(const ROBIN_READDIR *data)
{
size_t ret = 0;
{
int oldret = ret;
...
int oldret = ret;
ret = 0;
ret += len_RobinDirentry((data)->last);
ret += 1 + len_len(ret) + oldret;
}
if((data)->token){
int oldret = ret;
...
int
copy_ROBIN_READDIR(const ROBIN_READDIR *from, ROBIN_READDIR *to)
{
...
(to)->limit = NULL;
if ((from)->last) {
(to)->last = malloc(sizeof(*(to)->last));
if ((to)->last == NULL) return ENOMEM;
if (copy_RobinDirentry((from)->last, (to)->last)) return ENOMEM;
} else
(to)->last = NULL;
if ((from)->token) {
