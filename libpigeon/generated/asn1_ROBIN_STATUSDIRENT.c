...
int
enc_ROBIN_STATUSDIRENT(unsigned char *p, size_t len, const ROBIN_STATUSDIRENT *data, size_t *size)
{
size_t ret = 0;
size_t l;
int i, e;
...
int oldret = ret;
ret = 0;
e = enc_RobinDirentries(p, len, &(data)->entries, &l);
BACK;
...
BACK;
e = tb_der_put_len_and_tag(p, len, ret, ASN1_C_APPL, CONS, 28, &l);
BACK;
*size = ret;
return 0;
}
...
int
dec_ROBIN_STATUSDIRENT(const unsigned char *p, size_t len, ROBIN_STATUSDIRENT *data, size_t *size)
{
...
reallen = 0;
e = tb_der_match_tag_and_len(p, len, ASN1_C_APPL, CONS, 28, &reallen, &l);
FORW;
{
int dce_fix = 0;
...
e = dec_RobinDirentries(p, len, &(data)->entries, &l);
FORW;
if (dce_fix){
e = tb_der_match_tag_and_len(p, len, (Der_class)0, (Der_type)0, 0, &reallen, &l);
FORW;
...
return 0;
fail:
free_ROBIN_STATUSDIRENT(data);
return e;
}

void
free_ROBIN_STATUSDIRENT(ROBIN_STATUSDIRENT *data)
{
free_RobinStatus(&(data)->rep);
free_RobinDirentries(&(data)->entries);
if ((data)->lock) {
...
size_t
len_ROBIN_STATUSDIRENT(const ROBIN_STATUSDIRENT *data)
{
size_t ret = 0;
{
int oldret = ret;
...
int oldret = ret;
ret = 0;
ret += len_RobinDirentries(&(data)->entries);
ret += 1 + len_len(ret) + oldret;
}
if((data)->lock){
int oldret = ret;
...
int
copy_ROBIN_STATUSDIRENT(const ROBIN_STATUSDIRENT *from, ROBIN_STATUSDIRENT *to)
{
if (copy_RobinStatus(&(from)->rep, &(to)->rep)) return ENOMEM;
if (copy_RobinDirentries(&(from)->entries, &(to)->entries)) return ENOMEM;
if ((from)->lock) {
