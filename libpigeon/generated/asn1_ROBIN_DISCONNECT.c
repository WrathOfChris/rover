...
int
enc_ROBIN_DISCONNECT(unsigned char *p, size_t len, const ROBIN_DISCONNECT *data, size_t *size)
{
size_t ret = 0;
size_t l;
int i, e;
...
int oldret = ret;
ret = 0;
e = enc_RobinTime(p, len, &(data)->time, &l);
BACK;
...
int oldret = ret;
ret = 0;
e = enc_RobinConFlags(p, len, &(data)->flags, &l);
BACK;
...
BACK;
e = tb_der_put_len_and_tag(p, len, ret, ASN1_C_APPL, CONS, 14, &l);
BACK;
*size = ret;
return 0;
}
...
int
dec_ROBIN_DISCONNECT(const unsigned char *p, size_t len, ROBIN_DISCONNECT *data, size_t *size)
{
...
reallen = 0;
e = tb_der_match_tag_and_len(p, len, ASN1_C_APPL, CONS, 14, &reallen, &l);
FORW;
{
int dce_fix = 0;
...
e = dec_RobinConFlags(p, len, &(data)->flags, &l);
FORW;
if (dce_fix){
e = tb_der_match_tag_and_len(p, len, (Der_class)0, (Der_type)0, 0, &reallen, &l);
FORW;
...
e = dec_RobinTime(p, len, &(data)->time, &l);
FORW;
if (dce_fix){
e = tb_der_match_tag_and_len(p, len, (Der_class)0, (Der_type)0, 0, &reallen, &l);
FORW;
...
return 0;
fail:
free_ROBIN_DISCONNECT(data);
return e;
}

void
free_ROBIN_DISCONNECT(ROBIN_DISCONNECT *data)
{
free_RobinHeader(&(data)->hdr);
free_RobinConFlags(&(data)->flags);
free_RobinTime(&(data)->time);
}

size_t
len_ROBIN_DISCONNECT(const ROBIN_DISCONNECT *data)
{
size_t ret = 0;
{
int oldret = ret;
...
int oldret = ret;
ret = 0;
ret += len_RobinConFlags(&(data)->flags);
ret += 1 + len_len(ret) + oldret;
}
{
int oldret = ret;
ret = 0;
ret += len_RobinTime(&(data)->time);
ret += 1 + len_len(ret) + oldret;
}
ret += 1 + len_len(ret);
ret += 1 + len_len (ret);
...
int
copy_ROBIN_DISCONNECT(const ROBIN_DISCONNECT *from, ROBIN_DISCONNECT *to)
{
if (copy_RobinHeader(&(from)->hdr, &(to)->hdr)) return ENOMEM;
if (copy_RobinConFlags(&(from)->flags, &(to)->flags)) return ENOMEM;
if (copy_RobinTime(&(from)->time, &(to)->time)) return ENOMEM;
return 0;
}
