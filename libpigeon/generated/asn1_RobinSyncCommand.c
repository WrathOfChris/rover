...
int
enc_RobinSyncCommand(unsigned char *p, size_t len, const RobinSyncCommand *data, size_t *size)
{
size_t ret = 0;
size_t l;
int i, e;
...
int oldret = ret;
ret = 0;
e = enc_RobinData(p, len, &(data)->data, &l);
BACK;
...
int oldret = ret;
ret = 0;
e = enc_int64(p, len, &(data)->other, &l);
BACK;
...
int oldret = ret;
ret = 0;
e = enc_ROBIN_SYNCTYPE(p, len, &(data)->command, &l);
BACK;
...
int
dec_RobinSyncCommand(const unsigned char *p, size_t len, RobinSyncCommand *data, size_t *size)
{
...
e = dec_ROBIN_SYNCTYPE(p, len, &(data)->command, &l);
FORW;
if (dce_fix){
...
e = dec_int64(p, len, &(data)->other, &l);
FORW;
if (dce_fix){
e = tb_der_match_tag_and_len(p, len, (Der_class)0, (Der_type)0, 0, &reallen, &l);
FORW;
...
e = dec_RobinData(p, len, &(data)->data, &l);
FORW;
if (dce_fix){
e = tb_der_match_tag_and_len(p, len, (Der_class)0, (Der_type)0, 0, &reallen, &l);
FORW;
...
return 0;
fail:
free_RobinSyncCommand(data);
return e;
}

void
free_RobinSyncCommand(RobinSyncCommand *data)
{
free_ROBIN_SYNCTYPE(&(data)->command);
free_RobinData(&(data)->data);
}

size_t
len_RobinSyncCommand(const RobinSyncCommand *data)
{
size_t ret = 0;
{
int oldret = ret;
ret = 0;
ret += len_ROBIN_SYNCTYPE(&(data)->command);
...
int oldret = ret;
ret = 0;
ret += len_int64(&(data)->other);
ret += 1 + len_len(ret) + oldret;
}
{
int oldret = ret;
ret = 0;
ret += len_RobinData(&(data)->data);
ret += 1 + len_len(ret) + oldret;
}
ret += 1 + len_len(ret);
return ret;
...
int
copy_RobinSyncCommand(const RobinSyncCommand *from, RobinSyncCommand *to)
{
if (copy_ROBIN_SYNCTYPE(&(from)->command, &(to)->command)) return ENOMEM;
*(&(to)->offset) = *(&(from)->offset);
*(&(to)->length) = *(&(from)->length);
*(&(to)->other) = *(&(from)->other);
if (copy_RobinData(&(from)->data, &(to)->data)) return ENOMEM;
return 0;
}
