...
int
enc_RobinDisks(unsigned char *p, size_t len, const RobinDisks *data, size_t *size)
{
size_t ret = 0;
size_t l;
int i, e;
...
int oldret = ret;
ret = 0;
e = enc_RobinDisk(p, len, &(data)->val[i], &l);
BACK;
ret += oldret;
}
...
int
dec_RobinDisks(const unsigned char *p, size_t len, RobinDisks *data, size_t *size)
{
...
e = dec_RobinDisk(p, len, &(data)->val[(data)->len-1], &l);
FORW;
len = origlen - ret;
}
ret += oldret;
...
return 0;
fail:
free_RobinDisks(data);
return e;
}

void
free_RobinDisks(RobinDisks *data)
{
while ((data)->len){
free_RobinDisk(&(data)->val[(data)->len-1]);
(data)->len--;
}
free((data)->val);
(data)->val = NULL;
...
size_t
len_RobinDisks(const RobinDisks *data)
{
size_t ret = 0;
{
int oldret = ret;
...
int
copy_RobinDisks(const RobinDisks *from, RobinDisks *to)
{
if (((to)->val = malloc((from)->len * sizeof(*(to)->val))) == NULL && (from)->len != 0)
return ENOMEM;
for ((to)->len = 0; (to)->len < (from)->len; (to)->len++){
if (copy_RobinDisk(&(from)->val[(to)->len], &(to)->val[(to)->len])) return ENOMEM;
}
return 0;
}

RobinDisk *
add_RobinDisk(RobinDisks *data)
{
	RobinDisk *this;
	if (!data) return NULL;
...
int
del_RobinDisk(RobinDisks *data, RobinDisk *del)
{
	unsigned int u;
	RobinDisk *this;
