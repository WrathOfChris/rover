...
int
enc_RobinHostAddress(unsigned char *p, size_t len, const RobinHostAddress *data, size_t *size)
{
size_t ret = 0;
size_t l;
int i, e;
...
int oldret = ret;
ret = 0;
e = enc_bytes(p, len, &(data)->address, &l);
BACK;
...
int oldret = ret;
ret = 0;
e = enc_ADDR_TYPE(p, len, &(data)->addr_type, &l);
BACK;
...
int
dec_RobinHostAddress(const unsigned char *p, size_t len, RobinHostAddress *data, size_t *size)
{
...
e = dec_ADDR_TYPE(p, len, &(data)->addr_type, &l);
FORW;
if (dce_fix){
...
e = dec_bytes(p, len, &(data)->address, &l);
FORW;
if (dce_fix){
e = tb_der_match_tag_and_len(p, len, (Der_class)0, (Der_type)0, 0, &reallen, &l);
FORW;
...
return 0;
fail:
free_RobinHostAddress(data);
return e;
}

void
free_RobinHostAddress(RobinHostAddress *data)
{
free_ADDR_TYPE(&(data)->addr_type);
free_bytes(&(data)->address);
}

size_t
len_RobinHostAddress(const RobinHostAddress *data)
{
size_t ret = 0;
{
int oldret = ret;
ret = 0;
ret += len_ADDR_TYPE(&(data)->addr_type);
ret += 1 + len_len(ret) + oldret;
}
{
int oldret = ret;
ret = 0;
ret += len_int32(&(data)->port);
ret += 1 + len_len(ret) + oldret;
}
{
int oldret = ret;
ret = 0;
ret += len_bytes(&(data)->address);
ret += 1 + len_len(ret) + oldret;
}
ret += 1 + len_len(ret);
return ret;
...
int
copy_RobinHostAddress(const RobinHostAddress *from, RobinHostAddress *to)
{
if (copy_ADDR_TYPE(&(from)->addr_type, &(to)->addr_type)) return ENOMEM;
*(&(to)->port) = *(&(from)->port);
if (copy_bytes(&(from)->address, &(to)->address)) return ENOMEM;
return 0;
}
