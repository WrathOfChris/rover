...
int
enc_RobinGraft(unsigned char *p, size_t len, const RobinGraft *data, size_t *size)
{
size_t ret = 0;
size_t l;
int i, e;
...
int oldret = ret;
ret = 0;
e = enc_RobinCell(p, len, (data)->cell, &l);
BACK;
...
int oldret = ret;
ret = 0;
e = enc_RobinGraftFlags(p, len, &(data)->flags, &l);
BACK;
...
int
dec_RobinGraft(const unsigned char *p, size_t len, RobinGraft *data, size_t *size)
{
...
e = dec_RobinGraftFlags(p, len, &(data)->flags, &l);
FORW;
if (dce_fix){
e = tb_der_match_tag_and_len(p, len, (Der_class)0, (Der_type)0, 0, &reallen, &l);
FORW;
...
(data)->cell = malloc(sizeof(*(data)->cell));
if ((data)->cell == NULL) return ENOMEM;
e = dec_RobinCell(p, len, (data)->cell, &l);
FORW;
if (dce_fix){
e = tb_der_match_tag_and_len(p, len, (Der_class)0, (Der_type)0, 0, &reallen, &l);
FORW;
...
return 0;
fail:
free_RobinGraft(data);
return e;
}

void
free_RobinGraft(RobinGraft *data)
{
free_RobinHandle(&(data)->hdl);
free_ROBIN_TYPE(&(data)->type);
free_RobinGraftFlags(&(data)->flags);
if ((data)->cell) {
free_RobinCell((data)->cell);
free((data)->cell);
(data)->cell = NULL;
}
}
...
size_t
len_RobinGraft(const RobinGraft *data)
{
size_t ret = 0;
{
int oldret = ret;
...
int oldret = ret;
ret = 0;
ret += len_RobinGraftFlags(&(data)->flags);
ret += 1 + len_len(ret) + oldret;
}
if((data)->cell){
int oldret = ret;
ret = 0;
ret += len_RobinCell((data)->cell);
ret += 1 + len_len(ret) + oldret;
}
ret += 1 + len_len(ret);
return ret;
...
int
copy_RobinGraft(const RobinGraft *from, RobinGraft *to)
{
if (copy_RobinHandle(&(from)->hdl, &(to)->hdl)) return ENOMEM;
if (copy_ROBIN_TYPE(&(from)->type, &(to)->type)) return ENOMEM;
if (copy_RobinGraftFlags(&(from)->flags, &(to)->flags)) return ENOMEM;
if ((from)->cell) {
(to)->cell = malloc(sizeof(*(to)->cell));
if ((to)->cell == NULL) return ENOMEM;
if (copy_RobinCell((from)->cell, (to)->cell)) return ENOMEM;
} else
(to)->cell = NULL;
return 0;
}
