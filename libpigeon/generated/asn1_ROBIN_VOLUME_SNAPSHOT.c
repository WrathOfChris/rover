...
int
enc_ROBIN_VOLUME_SNAPSHOT(unsigned char *p, size_t len, const ROBIN_VOLUME_SNAPSHOT *data, size_t *size)
{
size_t ret = 0;
size_t l;
int i, e;
...
int oldret = ret;
ret = 0;
e = enc_RobinHandle(p, len, (data)->dhdl, &l);
BACK;
...
BACK;
e = tb_der_put_len_and_tag(p, len, ret, ASN1_C_APPL, CONS, 107, &l);
BACK;
*size = ret;
return 0;
}
...
int
dec_ROBIN_VOLUME_SNAPSHOT(const unsigned char *p, size_t len, ROBIN_VOLUME_SNAPSHOT *data, size_t *size)
{
...
reallen = 0;
e = tb_der_match_tag_and_len(p, len, ASN1_C_APPL, CONS, 107, &reallen, &l);
FORW;
{
int dce_fix = 0;
...
(data)->dhdl = malloc(sizeof(*(data)->dhdl));
if ((data)->dhdl == NULL) return ENOMEM;
e = dec_RobinHandle(p, len, (data)->dhdl, &l);
FORW;
if (dce_fix){
e = tb_der_match_tag_and_len(p, len, (Der_class)0, (Der_type)0, 0, &reallen, &l);
FORW;
...
return 0;
fail:
free_ROBIN_VOLUME_SNAPSHOT(data);
return e;
}

void
free_ROBIN_VOLUME_SNAPSHOT(ROBIN_VOLUME_SNAPSHOT *data)
{
free_RobinHeader(&(data)->hdr);
free_RobinHandle(&(data)->vhdl);
if ((data)->dhdl) {
free_RobinHandle((data)->dhdl);
free((data)->dhdl);
(data)->dhdl = NULL;
}
}
...
size_t
len_ROBIN_VOLUME_SNAPSHOT(const ROBIN_VOLUME_SNAPSHOT *data)
{
size_t ret = 0;
{
int oldret = ret;
...
int
copy_ROBIN_VOLUME_SNAPSHOT(const ROBIN_VOLUME_SNAPSHOT *from, ROBIN_VOLUME_SNAPSHOT *to)
{
if (copy_RobinHeader(&(from)->hdr, &(to)->hdr)) return ENOMEM;
if (copy_RobinHandle(&(from)->vhdl, &(to)->vhdl)) return ENOMEM;
if ((from)->dhdl) {
(to)->dhdl = malloc(sizeof(*(to)->dhdl));
if ((to)->dhdl == NULL) return ENOMEM;
if (copy_RobinHandle((from)->dhdl, (to)->dhdl)) return ENOMEM;
} else
(to)->dhdl = NULL;
return 0;
}
