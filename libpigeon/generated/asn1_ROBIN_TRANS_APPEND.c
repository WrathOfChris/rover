...
int
enc_ROBIN_TRANS_APPEND(unsigned char *p, size_t len, const ROBIN_TRANS_APPEND *data, size_t *size)
{
size_t ret = 0;
size_t l;
int i, e;
...
BACK;
e = tb_der_put_len_and_tag(p, len, ret, ASN1_C_APPL, CONS, 55, &l);
BACK;
*size = ret;
return 0;
}
...
int
dec_ROBIN_TRANS_APPEND(const unsigned char *p, size_t len, ROBIN_TRANS_APPEND *data, size_t *size)
{
...
reallen = 0;
e = tb_der_match_tag_and_len(p, len, ASN1_C_APPL, CONS, 55, &reallen, &l);
FORW;
{
int dce_fix = 0;
...
size_t
len_ROBIN_TRANS_APPEND(const ROBIN_TRANS_APPEND *data)
{
size_t ret = 0;
{
int oldret = ret;
